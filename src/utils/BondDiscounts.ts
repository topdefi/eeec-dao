...
import {
  EEECETHLPBOND_CONTRACT,
  EEECETHLPBOND_CONTRACT_BLOCK,
  WETHBOND_CONTRACT,
...
export function updateBondDiscounts(transaction: Transaction): void{
    ...
    // EEEC-ETH
    if(transaction.blockNumber.gt(BigInt.fromString(EEECETHLPBOND_CONTRACT_BLOCK))){
        let bond = BondDepository.bind(Address.fromString(EEECETHLPBOND_CONTRACT))
        ...
        if(price_call.reverted===false && price_call.value.gt(BigInt.fromI32(0))){
            ...
            log.debug("EEECETH Discount EEEC price {}  Bond Price {}  Discount {}", [ohmRate.toString(), price_call.value.toString(), bd.squideth_discount.toString()])
        }
    }
    ...
    if(transaction.blockNumber.gt(BigInt.fromString(WETHBOND_CONTRACT_BLOCK))){
        ...
        if(price_call.reverted===false && price_call.value.gt(BigInt.fromI32(0))){
            ...
            log.debug("WETH Discount EEEC price {}  Bond Price {}  Discount {}", [ohmRate.toString(), price_call.value.toString(), bd.weth_discount.toString()])
        }
    }
