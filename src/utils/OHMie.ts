...
export function updateOhmieBalance(ohmie: Ohmie, transaction: Transaction): void{
    ...
    if(transaction.blockNumber.gt(BigInt.fromString(EEECETHLPBOND_CONTRACT_BLOCK))){
        let bondSquidETH_contract = BondDepository.bind(Address.fromString(EEECETHLPBOND_CONTRACT))
        ...
        if (pending.value1.gt(BigInt.fromString("0"))){
            ...
            let binfo = loadOrCreateContractInfo(ohmie.id + transaction.timestamp.toString() + "EEECETHLPBond")
            binfo.name = "EEEC-ETH"
            binfo.contract = EEECETHLPBOND_CONTRACT
            binfo.amount = pending_bond
            binfo.save()
            bonds.push(binfo.id)

            log.debug("Ohmie {} pending EEECETHLPBond {} on tx {}", [ohmie.id, toDecimal(pending.value1, 9).toString(), transaction.id])
        }
    }
